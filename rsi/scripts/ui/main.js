...
Example.Grid = Ext.extend(Ext.grid.GridPanel, {
	...
	,renderServerName:function(val, cell, record) {
		...
		html += '<div>';
		html += '<img qtip="'+qtip+'" src="resources/icons/server.png" height="24px" />'
		html += '</div>';
		html += '</td>';
		html += '<td>';
		html += '<div qtip="'+qtip+'">';
	...
	,renderServerType:function(val, cell, record) {
		...
		if (data.serverType=="lenovoThinkCentre") {
			imgSource = "resources/icons/lenovoThinkCentre.jpg";
		}
		else if (data.serverType=="T2000") {
			imgSource = "resources/icons/T2000.jpg";
		}
		else if (data.serverType=="T5120") {
			imgSource = "resources/icons/T5120.jpg";
		}
		else if (data.serverType=="T5220") {
			imgSource = "resources/icons/T5220.jpg";
		}
		else if (data.serverType=="T6320") {
			imgSource = "resources/icons/T6320.jpg";
		}
		else if (data.serverType=="T6340") {
			imgSource = "resources/icons/T6340.jpg";
		}
		html += '<div>';
		html += '<table border="0" cellpadding="2" cellspacing="0">';
		html += '<tr>';
...
function startHeapMonitor(url,panelId) {
	...
	var threadConfig = {
		request: requestObj
		,threadAction: "getHeapInfoAction('"+url+"','"+panelId+"')"
		,interval: interval
	};
...
function showPleaseWait(panelId,show) {
	...
	if (show) {
		panelObj.innerHTML = "<img src=\"resources/icons/loading.gif\" />";
	}
	else {
		panelObj.innerHTML = "";
	}
