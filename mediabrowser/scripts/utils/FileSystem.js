...
function fileExists(filePath) {
	var r = mozillaFileExists(filePath);
	if (r == null) {
		r = ieFileExists(filePath);
	}
	return r;
}

function listFiles(pathUrl) {
	var r = mozillaListFiles(pathUrl);
	if((r == null) || (r == false)) {
		r = ieListFiles(pathUrl);
	}
	return r;
}

function ieListFiles(path) {
	//declare locals
	var fso = null;
	var fldr = null;
	var fc = null;
	...
	try {
		...
		alert("ieListFiles(): Exception while attempting to create ActiveXObject.\n\n" + ex);
		return null;
	}
	if (fso.FolderExists(path)) {
		...
		for (; !fc.atEnd(); fc.moveNext()) {
			fileObj = [];
			filePath = String(fc.item().path);
			fileSize = String(fc.item().fileSize);
			lastModif = fc.item().lastModifiedTime;
...
function mozillaListFiles(_path) {
	if(window.Components) {
		try {
			...
			alert("mozillaListFiles(): Exception while attempting to list files\n\n" + ex);
			return false;
		}
	}
	return null;
...
function ieFileExists(filePath) {
	//declare locals
	var fso = null;
	...
	try {
		...
		alert("ieFileExists(): Exception while attempting to create ActiveXObject.\n\n" + ex);
		return null;
	}
}

function mozillaFileExists(filePath) {
	if(window.Components) {
		try {
			netscape.security.PrivilegeManager.enablePrivilege("UniversalXPConnect");
			var file = Components.classes["@mozilla.org/file/local;1"].createInstance(Components.interfaces.nsILocalFile);  
			...
			alert("mozillaFileExists(): Exception while attempting to check if file exists.\n\n" + ex);
			return null;
		}
	}
	return null;
...
function urlToPath(aPath) {
	if(window.Components) {
		try {
			...
			alert("urlToPath(): Exception while attempting to convert url to path.\n\n" + ex);
			return false;
		}
	}
	return null;
